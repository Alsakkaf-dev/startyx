...
(function (root) {
  ...
  var DOCUMENTS = [
    ...
    D("op.5.1.3.16", "أمر التوريد المخزني", {
      family: "توريد مخزني", layer: 4,
      role: "المستند الوحيد (غير المشتريات والتحويل) الذي يُدخل بضاعة للمخزن بتكلفة ويولّد قيداً — في بتروسبيشل هو مستند الإنتاج الفعلي (77٪ من قيمته من «مخزن تحت التشغيل»).",
      header: ["date","branch","warehouse","account(A_CODE)","costCenter","description"],
      lines: ["item","unit","qty","cost"],
      posting: { legs: [
        { side: "debit",  account: { map: "مخزون:مخزون" },    amount: "qtyValue" },
        { side: "credit", account: { from: "header.account" }, amount: "qtyValue",
          note: "حساب الرأس A_CODE يُحدَّد يدوياً أو من «ربط حساب أمر التوريد بالمجموعة»" }
      ]},
      validations: ["تكلفة الصنف > صفر.", "المبلغ لا يتجاوز الحد الأعلى للحساب إلا بتأكيد.", "رقم المرجع لا يتكرر إلا بتأكيد."],
      dependsOn: ["item","warehouse","costCenter"], feeds: ["stockMovement","journalEntry"]
      /* [قاعدة: GO/05-warehouse.md #op.5.1.3.16] القيد الفعلي على 426/426 سجلاً 2026: من حـ/1202010001
         المخزون السلعي إلى حـ/ A_CODE (يتفاوت بحسب نوع العملية: إنتاج · استلام بين الشركتين · فكّ كرتون...).
         كان مصنَّفاً خطأً "بانتظار المواصفة" رغم توثيقه الكامل ورغم أنه أصلاً موصول لخادم حقيقي (post-map.js) — صُحِّح 2026-09-23. */
    }),
  ...
  var NON_POSTING = [
    ...
    { ref: "op.5.1.3.2",  label: "إذن التوريد المخزني", becomes: "op.5.1.3.16",
      why: "[قاعدة: GO/05-warehouse.md IV-D27] محضر فحص واعتماد بلا أثر مخزني ولا محاسبي (صفر صفوف حقيقية 2026) — خرج من نطاق الـ56، حلّ محله op.5.1.3.16 الذي يحرّك المخزون ويولّد القيد فعلاً." }
  ];
