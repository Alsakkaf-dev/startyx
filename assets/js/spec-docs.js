  ...
  var DOCUMENTS = [
    ...
    D("op.7.5.3.6", "فاتورة المبيعات", {
      ...
      posting: {
        ...
        pendingDecision: {
          question: "هل المخزون مستمر (perpetual) أم دوري (periodic)؟",
          why: "يحدّد هل تُرحَّل التكلفة مع كل فاتورة أم عند الجرد فقط.",
          blockedBy: "cfg.6.1.1",
          note: "«معايير التقييم» ما زالت «قيد التحقق» في إعدادات بتروسبيشل. " +
                "إن كان المخزون دورياً فاحذف سطري التكلفة واحتسبها عند الإقفال."
        }
      },
      validations: [
    ...
    D("op.5.1.3.2", "إذن التوريد المخزني", {
      ...
      posting: {
        legs: [
          ...
          { side: "credit", account: { map: "مخزون:وسيط" },  amount: "qtyValue",
            note: "بضاعة وردت ولم تُفوتر — يُقفل عند ورود الفاتورة" }
        ],
        pendingDecision: {
          question: "هل الإذن يُرحَّل محاسبياً أم كمياً فقط؟",
          why: "إن كان كمياً فقط فلا قيد هنا، والقيد يأتي مع الفاتورة.",
          blockedBy: "cfg.6.1.1"
        }
      },
